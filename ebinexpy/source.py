...
class Ebinex:
    def __init__(self, username: str, password: str, keep=False, **kwargs) -> None:
        ...
        if not logger:
            ...
            logging.basicConfig(filename=logpath, filemode='a', format='[%(levelname) 5s/%(asctime)s] %(name)s: %(message)s', level=logging.INFO)
        ...
        if credentials:
            ...
            try:
                ...
                driver.find_element(By.NAME, 'keepLoggedIn').click(); Captcha(driver).resolve()
