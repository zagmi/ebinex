...
class Credentials:
    def __init__(self, account_id: str, access_token: str, config: 'EbinexConfig', expiration=DEFAULT_EXPIRATION):
...
class EbinexConfig:
    def __init__(self, symbol: str, timeframe: Timeframe, environment: Union[Environment, str]):
        self.symbol = symbol
        self.timeframe = timeframe
        self.environment = environment
    ...
    def to_dict(self) -> Dict[str, str]:
        return {
            ...
            'symbol': self.symbol,
            'environment': self.environment if isinstance(self.environment, str) else self.environment.name
        }
...
class EbinexSymbol:
    def __init__(self, symbol: str, symbolType: str, symbolLabel: str, marketStatus: str, openMarketTime: Optional[str], closeMarketTime: Optional[str],  payout: float, hrs24PercentualChange: float):
        self.symbol = symbol
...
class EbinexAccount:
    def __init__(self, id: str, environment: Environment, user_role: str, default_coin_balance: float, label: str):
        self.id = id
        self.environment = environment
...
class EbinexParameters:
    ...
    def from_list(cls, data: List[Dict[str, Any]]) -> 'EbinexParameters':
        import json; data_dict = {item['key']: item['value'] for item in data}
